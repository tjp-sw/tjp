...
void Fire() {
  ...
  // Step 1.  Cool down every cell a little
    for(int i = 0; i < RINGS_PER_NODE; i++) {
      ...
      int fireCooling = 32 + (i + loop_count) % 50;
    ...
    for(int i = 0; i < RINGS_PER_NODE; i++) {
      ...
      for(int j = 0; j < LEDS_PER_RING; j++) {
          leds_raw[get_idx(i,j)] = GetHeatColor(heat[i][j]);
      }
    }
}
...
void frequency_pulse_fancy() {
  ...
  for(uint8_t j = 0; j < NUM_CHANNELS-1; j++) {
    ...
    for(uint8_t ring = 0; ring < RINGS_PER_NODE; ring++) {
      /*if(i == white_ring) {
        for(uint16_t pixel = inner_start; pixel <= inner_end; pixel++)
          leds_raw[get_idx(i, pixel)] = whiteColor;
        
        for(uint16_t pixel = outer_start; pixel <= outer_end; pixel--)
          leds_raw[get_idx(i, pixel)] = whiteColor;
      }
      else if(i == white_ring - 1 || i == white_ring + 1 || (i == 0 && white_ring == RINGS_PER_NODE-1) || (i == RINGS_PER_NODE-1 && white_ring == 0)){
        for(uint16_t pixel = inner_start; pixel <= inner_end; pixel++)
          leds_raw[get_idx(i, pixel)] = midColor;
        ...
        for(uint16_t pixel = inner_start; pixel <= inner_end; pixel++)
          leds_raw[get_idx(ring, pixel)] = tempColor;
  ...
  for(uint8_t ring = 0; ring < RINGS_PER_NODE; ring++) {
    for(uint16_t pixel = inner_end+1; pixel < outer_start; pixel++)
      leds_raw[get_idx(ring, pixel)] = tempColor;
  }
}

#define PIX_PER_CHAN 28
void freq_pulse(){
  leds_all = CRGB::Black;
  ...
  CRGB my_colors[NUM_CHANNELS];  
  uint16_t values[NUM_CHANNELS];
  
  for(int i = 0; i < NUM_CHANNELS; i++){
    my_colors[i] = get_color_hsv(palette, i);
    values[i] = frequencies_max[i];
  }

  for(uint8_t ring = 0; ring < NUM_RINGS; ring++) {
    for(uint8_t chan = 0; chan < NUM_CHANNELS; chan++) {
      for(uint8_t pixel = chan * PIX_PER_CHAN; pixel <= (chan+1) * PIX_PER_CHAN; pixel++) {
        leds_raw[get_idx(ring, pixel)] = my_colors[chan];
      }
    }
  }
}
  ...
  for(uint8_t ring = node_number * RINGS_PER_NODE; ring < (node_number + 1) * RINGS_PER_NODE; ring++)
  {
    ...
    if(explodeRing) {
      for(uint16_t pixel = 0; pixel < LEDS_PER_RING; pixel++)
        leds_raw[get_idx(ring, pixel)] = CRGB::White; //flash white
    }
  }
}
*/
...
void variable_spin() {
  ...
  for(uint8_t ring = node_number*RINGS_PER_NODE; ring < (node_number+1)*RINGS_PER_NODE; ring++) {
    ...
    leds_raw[get_idx(ring, centerPoint)] = colors[ring < 6 ? ring : 11 - ring];
    if(centerPoint == 0)
      leds_raw[get_idx(ring, LEDS_PER_RING - 1)] = colors[ring < 6 ? ring : 11 - ring];
    else
      leds_raw[get_idx(ring, centerPoint-1)] = colors[ring < 6 ? ring : 11 - ring];

    if(centerPoint == LEDS_PER_RING - 1)
      leds_raw[get_idx(ring, 0)] = colors[ring < 6 ? ring : 11 - ring];
    else
      leds_raw[get_idx(ring, centerPoint+1)] = colors[ring < 6 ? ring : 11 - ring];
  }
}
...
*/
  ...
  for(uint8_t ring = node_number*RINGS_PER_NODE; ring < (node_number+1)*RINGS_PER_NODE; ring++)
  {
    ...
    for(uint16_t pixel = 0; pixel <= innerHeight; pixel++)
      leds_raw[get_idx(ring, pixel)] = thisColor;
    for(uint16_t pixel = LEDS_PER_RING - 1; pixel < LEDS_PER_RING - outerHeight; pixel++)
      leds_raw[get_idx(ring, pixel)] = thisColor;
    for(uint16_t pixel = HALF_RING - 1 - (centerBandWidth-1)/2; pixel < HALF_RING + centerBandWidth/2; pixel++)
      leds_raw[get_idx(ring, pixel)] = centerColor;
  }
}
...
void ScrollingGradient_TwoColor(uint8_t overlay) {
  ...
  const uint16_t extendedLEDCount = ((NUM_LEDS-1)/period+1)*period;
  ...
  for(uint16_t i = 0; i < extendedLEDCount; i++) {
    uint16_t idx = (loop_count/2 + i) % extendedLEDCount;
    ...
    for(uint8_t ring = node_number*RINGS_PER_NODE; ring < (node_number+1)*RINGS_PER_NODE; ring++) {
      leds_raw[get_idx(ring, idx)] = temp;
    }
  }
