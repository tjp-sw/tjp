...
void draw_debug_mode() {
  ...
  uint8_t unlitPixels = 8;
  uint8_t speed_damp = node_number < NUM_NODES ? 1 : 15;

  leds_all = CRGB::Black;
  
  for(uint8_t ring = 0; ring < ringOffset + RINGS_PER_NODE; ring++)
  {
    // Pick color based on ring
    uint8_t temp = ring % 4;
    CRGB ringColor = temp == 0 ? get_color(show_parameters[PALETTE_INDEX], 0) : temp == 1 ? get_color(show_parameters[PALETTE_INDEX], 2) : temp == 2 ? get_color(show_parameters[PALETTE_INDEX], 4) : get_color(show_parameters[PALETTE_INDEX], 6);
    
    // Determine strand number and number of pixels to light up
    if(ring - ringOffset < (RINGS_PER_NODE/2)) {
      temp = ring % 2;
    }
    else {
      temp = 2 + ring % 2;
    ...
    for(uint16_t curPixel = 0; curPixel < extendedLEDCount; curPixel++) {
      uint16_t idx = (curPixel + loop_count/speed_damp) % extendedLEDCount; // Add scrolling based off loop_count
      ...
      if(curPixel % period < litPixels) { // This line references curPixel since we are talking about the pattern
        leds_raw[get_idx(ring+ringOffset,idx)] = ringColor; // This line references idx since we are talking about where the pattern gets written
      }
    }
  }
}
