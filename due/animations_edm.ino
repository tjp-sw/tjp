...
inline void equalizer_full(uint8_t display_mode) {
...
inline void equalizer_variable(uint8_t display_mode) {
  ...
  while(cur_height < stop_point) {
    ...
    for(uint8_t ring = 0; ring < RINGS_PER_NODE; ring++) {
      leds[get_1d_index(ring, cur_height)].fadeToBlackBy(32);// = CRGB::Black;
      if(display_mode == DISPLAY_SPLIT) { leds[get_1d_index(ring, LEDS_PER_RING - 1 - cur_height)].fadeToBlackBy(32); }
    }
    cur_height++;
  }
}
