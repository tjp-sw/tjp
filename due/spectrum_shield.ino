...
void read_frequencies() {
  ...
  for(int freq_amp = 0; freq_amp<NUM_CHANNELS; freq_amp++)
  {
    ...
    if (frequencies_one[freq_amp] > NOISE_REDUCT){
     frequencies_one[freq_amp] = frequencies_one[freq_amp] - NOISE_REDUCT;
    } else {
     frequencies_one[freq_amp] = 0;
    }
    
    // Calibrate intensity - Channel 2  --------------------------------//  
    if (frequencies_two[freq_amp] > NOISE_REDUCT){
      frequencies_two[freq_amp] = frequencies_two[freq_amp] - NOISE_REDUCT;
    } else {
