    ...
    for (int ring = node_number*RINGS_PER_NODE; ring < (node_number+1)*RINGS_PER_NODE; ring++) {
      for (int pixel = 0; pixel < LEDS_PER_RING; pixel++) {
        ...
        if (pixel % strip_length < color_length) {
          leds_raw[get_idx(ring, (pixel + rotation_direction * loop_count + offset*ring) % LEDS_PER_RING)] = get_color(palette_num, show_colors[0]);
        }

        // second color for color_length pixels
        else if (pixel % strip_length < 2*color_length) {
            leds_raw[get_idx(ring, (pixel + rotation_direction * loop_count + offset*ring) % LEDS_PER_RING)] = get_color(palette_num, show_colors[1]);
        }

        // third color for color_length pixels
        else if (pixel % strip_length < 3*color_length) {
            leds_raw[get_idx(ring, (pixel + rotation_direction * loop_count + offset*ring) % LEDS_PER_RING)] = get_color(palette_num, show_colors[2]);
        }
        // black color for color_length pixels
        else {
          leds_raw[get_idx(ring, (pixel + rotation_direction * loop_count + offset*ring) % LEDS_PER_RING)] = CRGB::Black;
        }
      }
    }
  }
...
void diane_arrow_1() {
  ...
  for (ring = 0; ring < HALF_RING / 3; ring++) {
    for (pixel = HALF_RING - 3 * ring; pixel <= HALF_RING + 3 * ring; pixel++) {
      if (pixel >= ring) {
        // inside the arrow
        leds_raw[get_idx((ring + loop_count) % NUM_RINGS, pixel)] = show_colors[0];
      }
      else {
        // background of arow
        leds_raw[get_idx((ring + loop_count) % NUM_RINGS, pixel)] = show_colors[1];
      }
    }
  }
...
void diane_arrow_2() {
  ...
  for (ring = 34; ring <= 35; ring++) {
    for (pixel = 0; pixel < LEDS_PER_RING; pixel++) {
      leds_raw[get_idx((34 + loop_count) % NUM_RINGS,pixel)] = show_colors[2];
    }
  }
  ...
  for (ring = HALF_RING / 6 + 2; ring < 2 * HALF_RING / 6 + 2; ring++) {
    for (pixel = HALF_RING - 6 * ring; pixel <= HALF_RING + 6 * ring; pixel++) {
      if (pixel >= ring) {
        ...
        // background of arrow
        leds_raw[get_idx((ring + loop_count) % NUM_RINGS, pixel)] = show_colors[2];
      }
    }
  }
